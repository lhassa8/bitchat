...
class ChatViewModel: ObservableObject {
    ...
    @Published var networkHealth: NetworkHealthStatus = .disconnected
    @Published var networkMetrics: NetworkHealthMetrics = NetworkHealthMetrics(
        connectedPeers: 0,
        averageRSSI: -100,
    ...
    @Published var showNetworkHealth: Bool = false
    ...
    init() {
        ...
        // Set up network health monitor
        NetworkHealthMonitor.shared.meshService = meshService
        NetworkHealthMonitor.shared.deliveryTracker = DeliveryTracker.shared
        NetworkHealthMonitor.shared.offlineQueue = OfflineMessageQueue.shared
        
        // Subscribe to network health updates
        NetworkHealthMonitor.shared.healthStatusChanged
            .receive(on: DispatchQueue.main)
            .sink { [weak self] status in
                self?.networkHealth = status
            }
            .store(in: &cancellables)
        
        NetworkHealthMonitor.shared.metricsUpdated
            .receive(on: DispatchQueue.main)
    ...
    func sendMessage(_ content: String) {
        ...
        if let selectedPeer = selectedPrivateChatPeer {
            ...
            if let channel = messageChannel, channelKeys[channel] != nil {
                ...
                let connectedPeers = meshService.getAllConnectedPeerIDs()
                let expectedRecipients = max(1, connectedPeers.count)
                DeliveryTracker.shared.trackMessage(message, recipientID: "channel", recipientNickname: channel, isFavorite: false, expectedRecipients: expectedRecipients)
                
                // Record message sent for network health
                ...
                let connectedPeers = meshService.getAllConnectedPeerIDs()
                let expectedRecipients = max(1, connectedPeers.count)
                DeliveryTracker.shared.trackMessage(message, recipientID: "broadcast", recipientNickname: "public", isFavorite: false, expectedRecipients: expectedRecipients)
    ...
    func sendPrivateMessage(_ content: String, to peerID: String) {
        ...
        if connectedPeers.contains(peerID) {
            ...
            let priority: QueuedMessage.MessagePriority = isFavorite ? .high : .normal
            OfflineMessageQueue.shared.queueMessage(message, recipientID: peerID, recipientNickname: recipientNickname, priority: priority, isFavorite: isFavorite)
            
            // Update message status to indicate it's queued
            if var updatedMessage = privateChats[peerID]?.last {
                updatedMessage.deliveryStatus = .pending
                privateChats[peerID]?[privateChats[peerID]!.count - 1] = updatedMessage
                objectWillChange.send()
            }
        }
    }
...
extension ChatViewModel: BitchatDelegate {
    ...
    func didUpdatePeerList(_ peers: [String]) {
        ...
        // Process offline message queue for newly connected peers
        OfflineMessageQueue.shared.processQueue()
        
        // Update network health with new peer count
...
// MARK: - Bluetooth Recovery Delegate

extension ChatViewModel: BluetoothRecoveryDelegate {
    func bluetoothRecoveryEvent(_ event: BluetoothRecoveryEvent) {
        switch event {
        case .bluetoothRecovered:
            print("[UI] Bluetooth recovered successfully")
            // Add a system message to inform the user
            let systemMessage = BitchatMessage(
                sender: "system",
                content: "bluetooth connection restored. reconnecting to mesh network...",
                timestamp: Date(),
                isRelay: false
            )
            messages.append(systemMessage)
        ...
        case .bluetoothRecoveryFailed:
            print("[UI] Bluetooth recovery failed")
            // Add a system message to inform the user
            let systemMessage = BitchatMessage(
                sender: "system",
                content: "bluetooth recovery failed. please check your bluetooth settings and restart the app if needed.",
                timestamp: Date(),
                isRelay: false
            )
            messages.append(systemMessage)
            
        case .bluetoothStateChanged(let central, let peripheral):
            print("[UI] Bluetooth state changed - Central: \(central), Peripheral: \(peripheral)")
            // Only show state changes if they indicate problems
            if central == "poweredOff" || peripheral == "poweredOff" {
                let systemMessage = BitchatMessage(
                    sender: "system",
                    content: "bluetooth is turned off. please enable bluetooth to continue messaging.",
                    timestamp: Date(),
                    isRelay: false
                )
                messages.append(systemMessage)
            } else if central == "unauthorized" || peripheral == "unauthorized" {
                let systemMessage = BitchatMessage(
                    sender: "system",
                    content: "bluetooth permission denied. please allow bluetooth access in settings.",
                    timestamp: Date(),
                    isRelay: false
                )
                messages.append(systemMessage)
