...
class BluetoothMeshService: NSObject {
    ...
    // Bluetooth state recovery properties
    ...
    private func cleanup() {
        ...
        stateRecoveryTimer?.invalidate()
        stateRecoveryTimer = nil
    }
    
    // MARK: - Bluetooth State Recovery
    
    private func startStateRecovery() {
        guard !isRecovering else { return }
        
        isRecovering = true
        recoveryAttempts = 0
        
        print("[StateRecovery] Starting Bluetooth state recovery")
        
        // Start recovery timer that checks every 2 seconds
        stateRecoveryTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { [weak self] _ in
            self?.attemptStateRecovery()
        }
    }
    
    private func attemptStateRecovery() {
        guard isRecovering && recoveryAttempts < maxRecoveryAttempts else {
            stopStateRecovery()
            return
        }
        
        recoveryAttempts += 1
        print("[StateRecovery] Recovery attempt \(recoveryAttempts)/\(maxRecoveryAttempts)")
        
        // Check if both managers are now powered on
        if centralManagerState == .poweredOn && peripheralManagerState == .poweredOn {
            print("[StateRecovery] Both managers powered on - recovery successful")
            stopStateRecovery()
            onBluetoothFullyRecovered()
            return
        }
        
        // Try to reinitialize managers if they're in a bad state
        if centralManagerState == .unsupported || centralManagerState == .unauthorized {
            print("[StateRecovery] Central manager in unsupported/unauthorized state - cannot recover")
            stopStateRecovery()
            return
        }
        
        if peripheralManagerState == .unsupported || peripheralManagerState == .unauthorized {
            print("[StateRecovery] Peripheral manager in unsupported/unauthorized state - cannot recover")
            stopStateRecovery()
            return
        }
        
        // For other states (poweredOff, resetting, unknown), wait for system to recover
        if centralManagerState == .poweredOff || peripheralManagerState == .poweredOff {
            print("[StateRecovery] Bluetooth is powered off - waiting for user to enable")
            // Continue waiting, don't give up yet
        }
        
        if centralManagerState == .resetting || peripheralManagerState == .resetting {
            print("[StateRecovery] Bluetooth is resetting - waiting for completion")
            // Continue waiting for reset to complete
        }
    }
    
    private func stopStateRecovery() {
        guard isRecovering else { return }
        ...
        if recoveryAttempts >= maxRecoveryAttempts {
            print("[StateRecovery] Recovery failed after \(maxRecoveryAttempts) attempts")
            notifyDelegate(about: .bluetoothRecoveryFailed)
        }
    }
    
    private func onBluetoothFullyRecovered() {
        print("[StateRecovery] Bluetooth fully recovered - reinitializing services")
        ...
        // Send broadcast announce to rejoin the network
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            self?.sendBroadcastAnnounce()
        }
        
        // Notify delegate of successful recovery
        notifyDelegate(about: .bluetoothRecovered)
    }
    
    private func notifyDelegate(about event: BluetoothRecoveryEvent) {
        DispatchQueue.main.async { [weak self] in
            if let delegate = self?.delegate as? BluetoothRecoveryDelegate {
                delegate.bluetoothRecoveryEvent(event)
            }
        }
    }
...
extension BluetoothMeshService: CBCentralManagerDelegate {
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        let previousState = centralManagerState
        centralManagerState = central.state
        
        print("[StateRecovery] Central manager state changed: \(previousState) -> \(central.state)")
        
        // Notify delegate of state change
        let centralStateString = stateToString(central.state)
        let peripheralStateString = stateToString(peripheralManagerState)
        notifyDelegate(about: .bluetoothStateChanged(central: centralStateString, peripheral: peripheralStateString))
        
        switch central.state {
        case .poweredOn:
            // If we were recovering and central is now on, check if we can complete recovery
            if isRecovering && peripheralManagerState == .poweredOn {
                print("[StateRecovery] Central powered on during recovery - checking full recovery")
                onBluetoothFullyRecovered()
        ...
        case .poweredOff, .resetting, .unknown:
            // Start recovery if we're not already recovering
            if !isRecovering && previousState == .poweredOn {
                print("[StateRecovery] Central manager went from powered on to \(central.state) - starting recovery")
                startStateRecovery()
            }
        ...
        @unknown default:
            print("[StateRecovery] Central manager in unknown state: \(central.state)")
        }
    }
...
extension BluetoothMeshService: CBPeripheralManagerDelegate {
    func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        let previousState = peripheralManagerState
        peripheralManagerState = peripheral.state
        
        print("[StateRecovery] Peripheral manager state changed: \(previousState) -> \(peripheral.state)")
        
        // Notify delegate of state change
        let centralStateString = stateToString(centralManagerState)
        let peripheralStateString = stateToString(peripheral.state)
        notifyDelegate(about: .bluetoothStateChanged(central: centralStateString, peripheral: peripheralStateString))
        
        switch peripheral.state {
        case .poweredOn:
            // If we were recovering and peripheral is now on, check if we can complete recovery
            if isRecovering && centralManagerState == .poweredOn {
                print("[StateRecovery] Peripheral powered on during recovery - checking full recovery")
                onBluetoothFullyRecovered()
            } else if !isRecovering {
                // Normal startup
                setupPeripheral()
        ...
        case .poweredOff, .resetting, .unknown:
            // Start recovery if we're not already recovering
            if !isRecovering && previousState == .poweredOn {
                print("[StateRecovery] Peripheral manager went from powered on to \(peripheral.state) - starting recovery")
                startStateRecovery()
            }
        ...
        @unknown default:
            print("[StateRecovery] Peripheral manager in unknown state: \(peripheral.state)")
        }
    }
